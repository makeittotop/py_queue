...
from job_spooler2.bs_jobSpooler.qjobSpooler_UI import Ui_qSpooler
...
class bs_jobSpooler(QtGui.QDialog):
    ...
    def genTask(self, internal=True):
        ...
        for s in jobRange:
            ...
            if self.ui.comment_line_edit.text() and str(self.ui.comment_line_edit.text()).strip() is not '':
			    jobComment = '{# %s } ' % (self.ui.comment_line_edit.text())
            else:
			    import getpass
			    jobComment = '{# Task created by %s } ' % (getpass.getuser())
